...
import org.matsim.contrib.spatialDrt.scheduler.RequestInsertionScheduler;
...
public class DefaultUnplannedRequestInserter implements UnplannedRequestInserter, MobsimBeforeCleanupListener {
	...
	private final RequestInsertionScheduler scheduler;
	...
	@Inject
	public DefaultUnplannedRequestInserter(DrtConfigGroup drtCfg, @Drt Fleet fleet, MobsimTimer mobsimTimer, EventsManager eventsManager, RequestInsertionScheduler scheduler,
										   VehicleData.EntryFactory vehicleDataEntryFactory, PrecalculablePathDataProvider pathDataProvider, InsertionCostCalculator.PenaltyCalculator penaltyCalculator) {
		this.drtCfg = drtCfg;
		this.fleet = fleet;
		...
		insertionProblem = new ParallelMultiVehicleInsertionProblem(pathDataProvider, drtCfg, mobsimTimer, forkJoinPool,
				penaltyCalculator, new SpatialDrtInsertionCostCalculater(mobsimTimer,penaltyCalculator));
		scheduler.initSchedules(drtCfg.isChangeStartLinkToLastLinkInSchedule());
	}
	...
	public void scheduleUnplannedRequests(Collection<DrtRequest> unplannedRequests) {
		...
		while (reqIter.hasNext()) {
			AtodRequest req = (AtodRequest) reqIter.next();
			VehicleData vData = new VehicleData(mobsimTimer.getTimeOfDay(), ((FleetImpl)fleet).getVehicles(req.getMode()).values().stream(), vehicleDataEntryFactory, forkJoinPool);
			Optional<SingleVehicleInsertionProblem.BestInsertion> best = insertionProblem.findBestInsertion(req, vData.getEntries());
			if (!best.isPresent()) {
				req.setRejected(true);
				eventsManager.processEvent(new DrtRequestRejectedEvent(mobsimTimer.getTimeOfDay(), req.getId(), NO_INSERTION_FOUND_CAUSE));
				...
				scheduler.scheduleRequest(bestInsertion.vehicleEntry, req, bestInsertion.insertion);
