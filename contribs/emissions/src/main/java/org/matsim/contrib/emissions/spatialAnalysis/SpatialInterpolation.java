...
public class SpatialInterpolation {
	...
	public SpatialInterpolation(final SpatialDataInputs inputs, final String outputFolder, final boolean isFilteringCellForShape) {
		...
		if( this.isFilteringCellForShape ){
			if(inputs.shapeFile!=null){
				...
				geoms = getSimplifiedGeometries(sfs);
	...
	private Collection<Geometry> getSimplifiedGeometries(Collection<SimpleFeature> features){
        Collection<Geometry> simplifiedGeoms = new ArrayList<>();
        int maxNumberOfVertices = 1000; //a constant, for analysis purpose, 1000 is a good value.
        for(SimpleFeature sf : features){
            Geometry outGeom = (Geometry) sf;
            double distanceTolerance = 1;
            int numberOfVertices = outGeom.getNumPoints();
            while (numberOfVertices > maxNumberOfVertices){
                outGeom = TopologyPreservingSimplifier.simplify(outGeom, distanceTolerance);
                numberOfVertices = outGeom.getNumPoints();
                distanceTolerance *= 10;
            }
            simplifiedGeoms.add( outGeom );
        }
	...
	public final void reset(){
		...
		this.grid.getGrid().values()
				 .stream()
				 .filter(this::isCellIncludedForInterpolation)
	...
	public void processLink(final Link link, final double intensityOnLink){
		...
		for(Point p: this.cellWeights.keySet()){
			...
			switch (inputs.getEmissionSourceMethod()) {
	...
	public void writeRData(final String fileNamePrefix, final boolean writingGGPLOTData){
		...
		if(inputs.isComparing){
			...
			fileName = outputFolder+"/rData"+"_"+fileNamePrefix+"_"+type+"_"+inputs.getCellWidth()+"_"+inputs.getEmissionSourceMethod()+"_"+scenarioCase+"_"+"diff"+".txt";
		} else {
			String scenarioCase = inputs.getInitialCase().split("/") [inputs.getInitialCase().split("/").length-1];
			fileName= outputFolder+"/rData"+"_"+fileNamePrefix+"_"+type+"_"+inputs.getCellWidth()+"_"+inputs.getEmissionSourceMethod()+"_"+scenarioCase+".txt";
		}
