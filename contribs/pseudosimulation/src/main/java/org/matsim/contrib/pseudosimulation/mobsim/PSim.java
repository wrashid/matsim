...
import org.matsim.contrib.pseudosimulation.searchacceleration.listeners.TransitStopInteractionListener.RouteStop;
import org.matsim.contrib.pseudosimulation.searchacceleration.listeners.TransitStopInteractionListener.RouteStopPerformances;
import org.matsim.contrib.pseudosimulation.searchacceleration.listeners.TransitStopInteractionListener.TripStopPerformance;
...
public class PSim implements Mobsim {
	...
	// TODO NEW
	private Map<TransitStopInteractionListener.RouteStop, RouteStopPerformances> routeStop2performances = null;
	...
	public PSim(Scenario sc, EventsManager eventsManager, Collection<Plan> plans, TravelTime carLinkTravelTimes,
			Map<TransitStopInteractionListener.RouteStop, RouteStopPerformances> route2stopPerformances) {
		this(sc, eventsManager, plans, carLinkTravelTimes);
		isUseTransit = true;
		// this.waitTimes = waitTimes;
		// this.stopStopTimes = stopStopTimes;
		// this.transitPerformance = transitPerformance;
		this.routeStop2performances = route2stopPerformances;
					...
					if (idx > 0) {
						...
						if (prevLeg.getMode().equals(TransportMode.car)) {
							...
							if (isUseTransit) {
								...
								// >>> TODO NEW >>>
								if (routeStop2performances != null) {
									final double currentTime_s = prevEndTime + travelTime;
									final TripStopPerformance accessPerformance = routeStop2performances
											.get(new RouteStop(line.getId(), transitRoute.getId(), accessStopId))
											.getNextInTime(currentTime_s);
									if (accessPerformance == null) {
										eventQueue.add(new PersonEntersVehicleEvent(Double.POSITIVE_INFINITY, personId,
												dummyVehicleId));
										eventQueue.add(new PersonLeavesVehicleEvent(Double.POSITIVE_INFINITY, personId,
												dummyVehicleId));
										travelTime = Double.POSITIVE_INFINITY;
									} else {
										eventQueue.add(new PersonEntersVehicleEvent(accessPerformance.stopArrivalTime_s, // stopDepartureTime_s,
												personId, dummyVehicleId));
										final TripStopPerformance egressPerformance = routeStop2performances
												.get(new RouteStop(line.getId(), transitRoute.getId(), egressStopId))
												.get(accessPerformance.departureId);
										if (egressPerformance == null) {
											eventQueue.add(new PersonLeavesVehicleEvent(Double.POSITIVE_INFINITY,
													personId, dummyVehicleId));
											travelTime = Double.POSITIVE_INFINITY;
										} else {
											eventQueue.add(new PersonLeavesVehicleEvent(
													egressPerformance.stopArrivalTime_s, personId, dummyVehicleId));
											travelTime = egressPerformance.stopArrivalTime_s - prevEndTime;
										}
									}
								} else
								// <<< TODO NEW <<<
						...
						if (Double.isInfinite(actEndTime)) {
							if (transitPerformance != null || routeStop2performances != null) {
