	...
	public static class Builder {
		...
		private void retrieveTypeSpecificCosts(Collection<CarrierVehicleType> vehicleTypes) {
			for(CarrierVehicleType type : vehicleTypes){
				typeSpecificCosts.put(type.getId().toString(), new VehicleTypeVarCosts(type.getVehicleCostInformation().perDistanceUnit, type.getVehicleCostInformation().perTransportTimeUnit)); //TODO: Are the other time variable costs also required here? KMT mar/18
			}
