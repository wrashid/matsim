...
public class NetworkReaderMatsimV1Test extends MatsimTestCase {
	...
	public void testAllowedModes_singleMode() {
		...
		TestCase.assertEquals("wrong number of allowed modes.", 1, modes.size());
		TestCase.assertTrue("wrong mode.", modes.contains(TransportMode.car));

		// make sure we do not just get some default-value back...
		link = prepareTestAllowedModes("bike");
		modes = link.getAllowedModes();
		TestCase.assertEquals("wrong number of allowed modes.", 1, modes.size());
		TestCase.assertTrue("wrong mode.", modes.contains(TransportMode.bike));
	}
	...
	public void testAllowedModes_emptyMode() {
		...
		Set<String> modes = link.getAllowedModes();
		TestCase.assertEquals("wrong number of allowed modes.", 0, modes.size());
	}
	...
	public void testAllowedModes_multipleModes() {
		...
		Set<String> modes = link.getAllowedModes();
		TestCase.assertEquals("wrong number of allowed modes.", 2, modes.size());
		TestCase.assertTrue("wrong mode.", modes.contains(TransportMode.car));
		TestCase.assertTrue("wrong mode.", modes.contains("bus"));

		link = prepareTestAllowedModes("bike,bus,walk");
		modes = link.getAllowedModes();
		TestCase.assertEquals("wrong number of allowed modes.", 3, modes.size());
		TestCase.assertTrue("wrong mode.", modes.contains(TransportMode.bike));
		TestCase.assertTrue("wrong mode.", modes.contains("bus"));
		TestCase.assertTrue("wrong mode.", modes.contains(TransportMode.walk));

		link = prepareTestAllowedModes("pt, train"); // test with space after comma
		modes = link.getAllowedModes();
		TestCase.assertEquals("wrong number of allowed modes.", 2, modes.size());
		TestCase.assertTrue("wrong mode.", modes.contains(TransportMode.pt));
		TestCase.assertTrue("wrong mode.", modes.contains("train"));
	}
	...
	private Link prepareTestAllowedModes(final String modes) {
		...
		// start test
		TestCase.assertEquals("expected one link.", 1, network.getLinks().size());
		Link link = network.getLinks().get(Id.create("1", Link.class));
		TestCase.assertNotNull("expected link with id=1.", link);
